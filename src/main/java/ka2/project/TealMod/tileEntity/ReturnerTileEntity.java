...
public class ReturnerTileEntity extends TileEntity {
	...
	// private HashSet<PlayerEntity> connectedPlayers = new
	// HashSet<PlayerEntity>();;
	...
	public HashSet<PlayerEntity> getPlayers() {
		...
		this.connectedUUIDs.forEach((u) -> {
			...
			if (world.getPlayerByUuid(u) != null) {
				connectedPlayers.add(world.getPlayerByUuid(u));
			}
		});
	...
	public CompoundNBT write(CompoundNBT compound) {
		/*
		 * if(world.isRemote()) { return super.write(compound); }
		 */
	...
	public void summon() {
		getPlayers().forEach((PlayerEntity p) -> {
			...
			if (!p.isSneaking()) {
				...
				p.teleportKeepLoaded(pos.getX() + 0.5, pos.getY() + 2, pos.getZ() + 0.5);
					...
					p.sendStatusMessage(ITextComponent.getTextComponentOrEmpty("Different dimmension"), false);

				((ServerPlayerEntity)p).changeDimension((ServerWorld) world);
				p.teleportKeepLoaded(pos.getX() + 0.5, pos.getY() + 2, pos.getZ() + 0.5);
				}
